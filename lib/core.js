...
dynamo.initialize = function(currentDirectory) {
    ...
    dynamo.app.configure(function() {
        ...
        dynamo.app.use(express.methodOverride());
        dynamo.app.use(bodyParser.json());
        dynamo.app.use(bodyParser.urlencoded());
        dynamo.app.use(express.methodOverride());
...
dynamo.run = function() {
    ...
    dynamo.server = server;
};

dynamo.runHTTP = function() {
    var http = require('http'),
        server = http.createServer(dynamo.app);
    dynamo.server = server;
    dynamo.server.listen(nconf.get('port'));
};

dynamo.getPort = function() {
